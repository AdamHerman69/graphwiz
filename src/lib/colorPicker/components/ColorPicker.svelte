<script lang="ts">
	...
	import type { A11yColor, Components } from '$lib/colorPicker/type/types';
	import {
		defaultTexts,
		type TextsPartial,
		type A11yTextsPartial
	} from '$lib/colorPicker/utils/texts';
	import { trapFocus, type Trap } from '$lib/colorPicker/utils/trapFocus';
	...
	function updateColor() {
		...
		if (hsv && (hsv.h !== _hsv.h || hsv.s !== _hsv.s || hsv.v !== _hsv.v || hsv.a !== _hsv.a)) {
			...
			rgb &&
			(rgb.r !== _rgb.r || rgb.g !== _rgb.g || rgb.b !== _rgb.b || rgb.a !== _rgb.a)
		) {
			color = colord(rgb);
			hex = color.toHex();
			hsv = color.toHsv();
...
>
	...
	<svelte:component this={getComponents().wrapper} bind:wrapper {isOpen} {isDialog}>
		{#if nullable}
			<svelte:component
				this={getComponents().nullabilityCheckbox}
				bind:isUndefined
				texts={getTexts()}
			/>
		{/if}
